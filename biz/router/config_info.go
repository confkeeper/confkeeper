...
func configInfoRoutes(apiGroup *gin.RouterGroup) {
	configGroup := apiGroup.Group("/config")
	{
		...
		configGroup.POST("/update_by_file", mw.JWTAuthMiddleware(utils.AuthOptions{IsShortTerm: true}), hConfigInfo.UpdateConfigByFile)
		...
		configGroup.GET("/get_by_file", mw.JWTAuthMiddleware(utils.AuthOptions{IsShortTerm: true}), hConfigInfo.GetConfigByFile)
		configGroup.HEAD("/get_by_file", mw.JWTAuthMiddleware(utils.AuthOptions{IsShortTerm: true}), hConfigInfo.GetConfigByFile)
