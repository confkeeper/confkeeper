...
type NacosUpdateConfigResp struct {
	handler.CommonResp
}
...
//	@Success		200			{object}	NacosUpdateConfigResp
...
func NacosUpdateConfig(c *gin.Context) {
	...
	if err != nil {
		c.JSON(http.StatusBadRequest, &NacosUpdateConfigResp{
	...
	if err := utils.IsAdmin(c); err != nil {
		...
		if err != nil || !hasPermission {
			c.JSON(http.StatusOK, &NacosUpdateConfigResp{
				CommonResp: handler.CommonResp{
